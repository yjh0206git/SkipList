...
namespace Thread{
    namespace Logger{
        ...
        public:
           void end_line(const Level&,const std::string&);
        private:
           std::mutex _mtx;
           std::tm _localtime;
        };
        ...
        private:
            FileLogger(const std::string& fn="create_at_" __DATE__ "-" __TIME__ ".log");
            void output(const std::tm&,const std::string&,const std::string&);
            ~FileLogger();
        private:
            std::ofstream _file;
        ...
        LoggerStream<T> BaseLogger<T>::operator()(Level level){
            return LoggerStream<T>(*this,level);
        }
        ...
        template<typename T>
        void BaseLogger<T>::end_line(const Level& level,const std::string& info){
